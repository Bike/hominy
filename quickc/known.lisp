...
(defun ptree-p (object)
  (typecase object
    ((or symbol null i:ignore) t)
    (cons (and (ptree-p (car object)) (ptree-p (cdr object))))
    (t nil)))
...
(defun compile-$vau (combinand cenv context)
  (destructuring-bind (ptree eparam . body) combinand
    (unless (ptree-p ptree)
      (warn "Bad syntax: $VAU given a non-ptree ~a" ptree)
    ...
    (let* ((cmod (asm:cmodule (cfunction context)))
           (res (compile-operative ptree eparam body cenv cmod))
           (info (info res))
           (cf (info:data info))
